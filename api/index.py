...
async def chat(req: ChatRequest):
    ...
    if groq_key and groq_key != "your_groq_api_key_here":
        try:
            ...
            msgs = [{"role": "system", "content": f"You are a CISO for {vertical.value}. Ask one short question at a time. After {MAX_TURNS} turns output JSON scorecard."}]
            for m in req.conversation_history:
                msgs.append({"role": m.role, "content": m.content})
            msgs.append({"role": "user", "content": req.message})
            want_json = user_turns >= MAX_TURNS - 1
            kwargs = {"model": os.getenv("GROQ_MODEL", "llama-3.1-8b-instant"), "messages": msgs, "max_tokens": 1024, "temperature": 0.3}
            if want_json:
